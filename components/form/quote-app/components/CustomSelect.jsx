...
// ─── CustomSelect ──────────────────────────────────────────────────────────────
export default function CustomSelect({ id, value, onChange, options, placeholder, disabled }) {
  const [open, setOpen] = useState(false);
  const selectedLabel = options.find((o) => o.id === value || o.value === value)?.label ?? null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={`qw-select-arrow${open ? " qw-select-arrow--open" : ""}`}>▾</span>
      </button>
          ...
          {options.map((opt) => {
            ...
            return (
              <li
                ...
                onMouseDown={() => { onChange(optValue); setOpen(false); }}
              >
                {optLabel}
                {optDescription && (
                  <span className="qw-select-option-desc">{optDescription}</span>
                )}
              </li>
            );
          })}
